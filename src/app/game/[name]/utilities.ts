...
export const getInitialSubText = (playerCount: number): string => {
  const options = [
    ...
    `${playerCount} player${playerCount !== 1 ? 's' : ''} think${playerCount !== 1 ? '' : 's'} they're ready. We'll see.`,
...
const getReadyOrRoundEndOptions = (paintersLeft: number): string[] => ([
...
export const getReadySubText = (paintersLeft: number): string => {
  const options = getReadyOrRoundEndOptions(paintersLeft);
  options.push(...[
...
export const getRoundEndSubText = (paintersLeft: number): string => {
  const options = getReadyOrRoundEndOptions(paintersLeft);
  options.push(...[
    ...
    `To think that a few seconds ago, there were still ${paintersLeft + 1} players waiting to draw...`
  ]);
